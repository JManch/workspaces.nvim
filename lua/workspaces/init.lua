...
M.add = function(path, name)
	if cd_types[name] ~= nil then
		notify.err(string.format("Workspaces cannot be named '%s'", name))
		return
	end
...
local find = function(name, path)
	...
	for i, workspace in ipairs(workspaces) do
		if workspace.name == name or (path and dir_eq(workspace.path, path)) then
			return workspace, i
		end
	end
...
---@param name string|nil
---@param cd_type string|nil
M.open = function(name, cd_type)
	if cd_type == nil then
		cd_type = "global"
	end
	...
	if not name then
		...
		vim.ui.select(workspaces, {
			prompt = "Select workspace to open " .. cd_type_prompts[cd_type] .. ":",
	...
	local cd_command = "cd"
	if cd_types[cd_type] ~= nil then
		cd_command = cd_types[cd_type]
	end
	...
	vim.api.nvim_create_user_command("WorkspacesOpen", function(cmd_opts)
		local parse_args = function()
			if #cmd_opts.fargs == 0 then
				return nil, nil
			elseif #cmd_opts.fargs == 1 then
				if cd_types[cmd_opts.fargs[1]] == nil then
					return cmd_opts.fargs[1], nil
				else
					return nil, cmd_opts.fargs[1]
				end
			else
				return unpack(cmd_opts.fargs)
			end
		end
		require("workspaces").open(parse_args())
	end, {
